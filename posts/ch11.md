...
而且，较小的质数通常都都能得到较漂亮的图形。 我注意到较小的 n 值可以让你避免变量给 deg 较大的质数。 我还没有完整验证过。(此处需要真实代码验证)


还有一项你可能想尝试的东西，就是给毛雷尔玫瑰用分数值。你完全不同改动函数。 你就直接传一个分数值给 maurer 函数。 因为我们总是从0 循环至 360， 我们不需要循环范围。下面是个传分数值的例子，在使用的时候确保分数值是分开传入 rose 函数的。
