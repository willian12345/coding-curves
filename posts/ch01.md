...
背景
多年来，我一直希望写一本名为“曲线编程”的书。 各种领人着迷的曲线可以用于有趣的编程、生成非常有趣的绘画。我开始写这本书至少三次了，这回冲动又来了，但这回我会现实一点，我不会想坐下从头到尾写一本书完整的书并把它编辑成册并自己出版。这不太可能。但我想开始写这部分相关的东西了。
...
计划
所以呢，我打算写成系列博客。我不知道发布频率有多高。勤快的话也许一周一篇。如果运气好一切顺利的话可能更频繁。当然也许会更少，如果我感到无聊、分心或忙于其它事情。但即使是有一点空，我还是会回来继续写。
也许，全部完成后真的可以成册。鬼知道呢。。
...
一般来说我们会限制在讨论二维的曲线。其中一些课题会用多篇博文.隨着内容的更新，目录也会更新。也许有更多的章节，我很open的，欢迎提建议。
...
我之前写的大多数文章会使用特定的编程语言，如： Playing With Chaos 内，使用的是javascript 和 HTML5  Canvas. 主要是希望能它能通用转换成的任意语言或平台， 所以我需要基于某个真实的编程语言来表示它。
...
但最近在阅读一些优秀的射线追踪相关的文章时，这些文章没有使用某种真实的编译语言实现，而是通过伪代码的方式展示例子。我改主意了，我这也会用伪代码作为代码示例。
...
所以，无论你在任何语言任何平台上实现，你需要注意以下几点必要条件：
A. 有能力设置canvas或其它类似可以提供绘制的能力
B. 在以上基础上可提供绘制不同颜色、粗细线条的能力
C.典型的编程语言特征和控制结构，如方法，变量，循环，条件等等 。可能还需要一些数组或列表还有对象结构类型，这样就可以创建一个像Point 类，拥有x 和y属性。 任何现代编程语言都包含提到的大多数特性，尽管可能样子看起来不太一样。
...
另外，如果能画圆（至少能画圆弧，圆弧能组成圆）和矩形且能为其填充颜色，当然如果能有填充文本的API就更好了。
简单的伪代码可能看起来像下面这样
...
我会避免深入面向对象或面向方法编程它们在不同平台上看起来太不一样了， 但我们又需要一些function ，所以我保留了一些最基本的：
...
首先，在定义foo方法前我先调用了foo ，在某些编程语言中可能会报错。如果报错，请根据你自己使用的语言调整调用顺序。

其次，绘制的API调用写的像是全局调用。在你的编程语言中可能需要在canvas 对象下引用，定义成全局或传进来。 我假设你自己能搞定，我就不多说了。在这里提出这些，是让你能转变思维，但这可不是最佳的编程实践。

当然我也不会在伪代码中关心 变量，参数，结果返回，除非需要明显说明或者特别重要。随着本系列的深入我的伪代码风格可能也会发生变化，如果是的话，我会回来改这里的说明的。安心了…
