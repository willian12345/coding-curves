...
我们先从简单的开始，但我会用几个不同的应用来演示。我不会深入详细讲解三角学，这里不会演示什么是90度角，邻边，对边，斜边和 soh-cah-toa(老外用来记忆三角函数的口诀)
...
如果你对上面一段我说的东西一无所知，那么放弃吧，先学上面我说的初中三角函数。
...
等等，我在干什么？我失去了一个不要脸的推荐我自己博客的机会。好吧， 这里是有关我讲解三角学的视频列表，这是最佳学习资料，比刚才的1.8亿搜索结果要好的多，可能帮你省不少钱。
...
依据你所使用编程语言对应的绘制api ，在使用lineTo api 前 你可能先需要使用 moveTo(0, height/2) 。当你搞定后应该会得到如下结果
...
这就是正弦波了，也许不如你预期的那样。有两个问题，但很好，因为它们都引出了我们要讨论的两个东西: 波长/频率 和 振幅。上图中太多波了--波长太短（或者说频率太高），波有点像点了- 振幅太低了。
...
振幅很容易拿捏。正弦值从 -1.0 至 1.0 ，我们仅需要将这些值乘以对应的振幅值就可以了。 canvas 高度乘以类似0.45这样的值可以让波形高度小于canvas的高度
...
振幅给了我们高度，但波还是太多了以致于我们不能说这就是正弦波。我们将在下面章节解决它。
...
#### 波长与频率
波长…. 等等, 波的长度，或者说两个连续相同位置点(一个周期)之间波的长度。在真实世界中，它们是真实测量的物理距离单位从米到纳米，取决于你测量的是何种波
...
波长与频率反向相关(反比)。短波(两个波形很近)等于高频(一段距离内拥有很多波形)。长波等于低频。
你可以应用两种测量方法用代码测试它们，让我们从频率开始。
...
## 频率
我们一般用“周期/每秒”(cps)来代指一段距离内的周期数。以收音机收到音频波举例，我们可以通过每一秒接收到了多少来统计。 专业名词“赫兹”简写为“Hz”代表每秒拥有多少个周期。100Hz 代表每秒100个周期。1千赫兹代表1000 cps, 1兆赫代表100万cps …
...
因为我们还没处理动态的波，这是因为这样更易理解测量给定的一个周期内波的频率。 上面显示的图形结果是根据上面我们写的例子代码，但我们是在800像素宽度内绘制正弦波，我们说的1频率，意为着一个正弦波图形应该穿越整个画布
...
我们替换之前给sin的x值，将x 乘以canvas宽度。 x / width 循环结果过程值会变为从 0.0 到 1.0。

然后我们再乘以 PI * 2 这会让值 0.0 至 6.28… , 这个非常像最初的例子中得到的结果。这个例子会让结果从0到1再到-1 然后回到0，刚好就是你看到的一个周期。

最后，我们乘以freq 频率。 当前设置的是1.0 ， 我们得到的仍然是一个周期。所有的这些设置结果就是下面的图形
...
好的，像那么回事儿了。但是一般我们常用的正弦图波形是先向上再向下的。如果你先看到的是先向下再向上，就如我们这里的例子中展示的那样，那是因为我们使用的y 轴是向下的与迪卡尔坐标系的y轴相反。 如果你了解如何使用绘图api 2d transform 方法, 你就能修复它。我将用用简单方式修复它，在调用 lineTo方法内将y 变成 height - y 就妥了
...
现在，你可以尝试改变amplitude 和 freq 这两个变量创建不同的波形图。下面是我将amplitude设为50 freq设为5的结果
...
现在，让我们用波长代替频率来重新实现一下。在这例子中，我们将用波长表示一个完整波形周期内占用的像素。 现在我们可以这样说：我想要一个100像素长的周期波形。
...
此番在sin函数调用时，我们将x 除以 wavelength。这样前100像素内，我们将得到值 0.0至1.0， 紧接着下一个100像素 1.0 至 2.0等 。

然后值再乘以 2PI，所以100像素内每一个像素都会乘以6.28 中的某一个值…并执行一个完整的波形周期
...
由于我们的canvas 画布宽 800像素 波长设置为 100 像素，我们如期得到了8个完整周期波型。
...
简单解释一下分辨率（译者注：不是物理上的分辨率，只是绘制过程中循环内的步幅）。这里，for循环每一次迭代在canvas画布中走过1像素距离， 这样让我们的正弦波看起来挺润的。但这会不会画的也太多了。如果在你的绘制工作中想减少一些线条绘制，你可以通过缩小减少分辨率来实现。我们创建一个叫 res 的变量 并将它添加进 x 在 for 循环内看看会发生什么 。 我们先从10 开始
...
在累加上，我们只绘制了之前绘制线条的10%。但你已经可以看到了线条有一点点块状化了。将res 加到 20 继续减少线条绘制，这回线条看起来就很粗糙了
...
余弦
在余弦上面我会讲快一占，因为基本知识已经在上面正弦中普及过了同样适用于余弦， 除了余弦图像相对正弦是翻转过来的。回到单循环波形简单的将sin 替换成 cos 就行。
...
这里，我们输入0.0 得到的是 1.0 然后往下掉到0.0， -1.0 再回到0.0 最后再回 1.0 结束。 就像是我们将0.0 慢慢变到 2 * PI .  就像是同样的波形转了 90 度（或都说 PI / 2 弧度）。如果我们将频率调高一点，更容易看清
...
至此学到的相关知识我们可以封装一个可利用的函数，它接收两个点参数在两点之间绘制一个正弦波。再次，我先放伪代码在下面，然后慢慢解释。
...
此函数接收6个参数，x 和 y 坐标点表示开始与结束点，还有频率和振幅。
我们通过dx 和 dy 获取计算两点间稳中各轴间的距离
...
我们先translate 移动到开始点并旋转到计算出来的角度.。 在这个点我们仅需要用 dist、freq、amp 这几个变量 绘制一个正弦波，正如我们之前所做的那样。
...
当我们的canvas 上下文移动到了我们希望的起始点时，我们就调用 lineTo(x, -y) 实现正确的正弦波绘制就像之前那样。
...
当搞定以上步骤后，调用stroke 来显示出路径还要调用pop（或 restore 或其它什么类似的 api ）恢复到函数绘制前的状态。
现在我们就可以像这样调用函数了
...
如果你正使用的编程语言没有transform 这一类函数，那你可太惨了。你依然可以实现类似的效果，但会非常复杂，这超出了本篇的范围。
...
在博文下方评论内有人提到 frequency 这一词可能会导致困惑，这词隐含的意思会是 A点B 点之间一共包含多少个周期循环，所以一个长波的绘制意谓着比同频绘制一个较短的 wavelength 要大。为解决这一点，你可能希望用wavelength 代替frequency. 这将确保所有具有相同波长的波看起来都很相似，无论它们有多长。另一个选项是将frequency与 frame 帧率联系起来，如：“每1000像素有多少个周期”，诸如此类的。无论怎么做都需要根据你自己的需求来，所以选择权交给你。

（译者注: 这是作者博文评论区有有人提出作者使用frequency 频率这个变量控制波形有歧义，因为按现在的sinewave函数在绘制两点距离不同时，所产生的图形，波长是不一样的。所以有人提议用wavelength 代替frequency 来实现，这样不管两点距离是多少，波长总是一样的，感觉作者的意思就是情况有很多，你可以根据自己的需求调整，调整很简单我试了一下, 循环中改成这句就行  y = sin(x / wavelength * PI * 2) * amp;  传 freq 变成 传 wavelegnth。源码中我用 wave-function-wavelength.html 演示了一下）
...
不像之前两个函数，正切自身的值没有限制在 -1至 1的范围，事实上，范围是无限的，让我们像之前对正弦那样输出查看一下正切值是啥样。
...
再一次，我们从0开始，但它迅速增长至14，接着直接跳到 -34，就很迷。这些值产不连续的，不是每步0.1为单位移动。 实际是这样的，随着输入值接近 PI/2 弧度（或90度角）数值快速增长到正无穷大，一旦超过了这个值，它又向下跳到负无穷，很快在PI （180 度角）又升到 0.0 ，然后在到达 2 * PI （360 度）前重复这一循环。这不是一个线性过程虽然它在接近和离开0时绘制了一条曲线，让我们用频率实现一个正切波形看看
...
和之前代码一样，只是将sin替换成了tan。我还减少了amplitude 值以更好的观察曲线。事实上，振幅用在这里解释可能用词不当，因为这里的振幅是无限的。但这个值的确会影响曲线形状。
...
有一点需要注意，这些接近垂直的线是从正无穷到负无穷。这些不是正切图像技术面上的部分。数学上来讲，这暗示着正切值在这条垂直线上有某个点会有某个值，但确不是。事实上在 90-270度之间这个值是 undefined 。这些值前后接近正负无穷。不确定这特性对你有没有用，但事实就是这样。
...
## 总结
非常好，我们已经学习了这个系列中最初部分的曲线。经过前面的内容学习后面我们将会在其它曲线中用到这里学习到的知识和相应的函数。 回头见。
