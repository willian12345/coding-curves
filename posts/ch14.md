...
最后，我猜 140 会是一个不错的半径值，它会将绘制出的图形限制在 600X600 大小的 canvas 内。事实上，我期望的是把图形限制在 canvas 大小的一半。但实际上大的叶子超出一部分也不影响。我们可以在代码中修复它，比如将半径乘以某些小数让大的叶子半径降下来。我就不做这部分限制了，我假装自己只会传小一点的合适的值，想要相关的限制你自己可以搞定的。
