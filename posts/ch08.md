...
你得确保你使用的编程语言用的是哪一个。通常你可以参考上面列出的几个例子，开始点用 moveTo 定义， 否则起点将会是绘图 api 最近一次的绘制点， 然后调用贝塞尔曲线函数定义控制点与结束点。你可以像下面这么做：
...
但有些编程语言可以允许你一次设定所有点。它是作为基础的内建函数，当然我们还是会忽略具体的 api 我们来自己实现一遍。
...
确保在 quadCurve 我们将起始点与结束点拆出来了，我们 moveTo 拆出第一个点，用 lineTo 拆出第最后一个点。 函数接受一个 res 参数，用于指定沿曲线上迭代多少次。我们将 t 初始值为 res 因为函数外已经移动到第一个点了，无论 t 值是否为 0。中间的所有点根据当前 t 绘制出线条。
...
我们的 quadBezierPoint 能用于实现动画，而内建函数做不到。在这一节， 就像之前章节我做的那样， 我已经假定你有或有能力实现无限循环的函数用于创建动画了。 还是叫它 loop 函数。 我不会像之前那样用 t 实现 0 到 1 绘制曲线，我将 让 t 从 0 到 finalT , finalT 的值会一直变化。
...
这样我们谅得到了 finalT 的当前值对应点的 x, y u并在 x, y 处画了个圆。假定你已经有了 circle 绘制函数。你如果有需要你可以在第三章里复制一个过来。
...
这些就是对贝塞尔曲线和路径的代码实现。但我这里还准备了一些其它有用的小技巧给你。
...
> 这很巧妙，但我希望曲线能穿过控制点。这就是我-大约在 2000 年左右
...
下图中红的是我用正常的二阶贝塞尔曲线画的，蓝的是用新函数画的。并且绘制了那些控制点用于证明。
...
有时被称为分段二阶贝塞尔曲线，我将展示一点简单的例子使用四阶曲线。它的实现并不难且它支持任意多的控制点。我甚至会为你创建一个曲线闭环的版本。
...
我不会深入太多，但我将覆盖基础部分并给你一些示例。

基本原则是在 “p0 p1 p2” 之间画创建一个中点，设为 pA。然后再创建一个p1 与 p2 之间的中点设为 pB 。连接p0 到 pA，然后使用pA，p1和pB绘制二阶贝塞尔曲线。
...
然后你又得到了p2 和 p3 之间的 pC 并且从 pB 绘制二阶贝塞尔曲线通过控制点 p2 到达 pC
...
继续以上步骤直到从倒数第二个中间点，穿过倒数第二点，结束在最后一个中间点。最终连接最后一个中间点到最后一个结束点。
...
实现这个效果的代码看起来有点儿棘手，但我已经弄过好几回了，我很高兴我已经有了像下面这样的函数。注意，为让让它能接受超大参数，它对需要传入多个坐标点需求非常有有用。无论它是类，结构，或拥有属性 x,y 的普通对象...。根据你自己使用的编程语言选择吧。此函数使用数组存储坐标点。假定数组应该有个 length 属性，在你的编程语言中有可能它不是一个属性，而是一个获取数组长度的方法。
...
如下例子中，我添加一半打（译者注：一打是 12 个 半打 是6 个）随机坐标点。我不知道你使用的编程语言中如何生成这些随机数，我假定你会有 randomPoint(xmin, ymin, xmax, ymax) 这样的函数。（事实上我在自已函数库中确实写过这样的函数！）一旦你有了这样的数组，把数组传进 multiCurve 后再调用 stroke 进行描边渲染：
...
相当不错。 曲线之所以看起来是样，其实在生成这些随机数的数组时也是要根据上下文环境来的
...
最后部分将要介绍的是如何将函数改造成支持曲线形成闭环。主要是去除掉开始与结束线断，并且将它首尾相连。
...
我们先从第一个中间点开始，循环剩下的点，找到各自点的中间点并用二阶贝塞尔曲线相连。这里留下下最后一个中间点
...
找到首尾间的中间点，然后将剩下两段曲线连在一起将形状闭合。如下图与上面一样的设置一样，但用 multiLoop 函数取代了之前的 multiCurve函数 （随机出的 points 数组值也不一样）。

![image](images/ch08/image-18.png);

These are two of my favorite functions, and I’m happy to share them with you.

这是我的两个最喜欢的函数，我很乐意分享给你们。
...
最终分布的不均。尾部间隔较大，中间间隔又比较紧。这就是在贝塞尔曲线上使用会产生的结果。所以我们得找到方法让这些点平均分布。

遗憾的是没有什么简单的方式来实现。我将用粗暴的方式强行实现它，当然也会给你一些用于优化它的提示。

为了在曲线上均分，直觉告诉我们需要先获取曲线的长度。如果长度是 200 像素， 且你想要在此长度的曲线上每间隔 10 像素放一个点共 20 个点。

意不意外，没什么公式可以在贝塞尔曲线实现这一效果。但们可以通过 point 数组元素每一个间的间距简化实现。会像下面代码：
...
这里，我们合计采样多少个点。采样越多，越精准。

然后 dt 是我们在曲线上循环迭代的步长。

我们追踪每一步循环的 x, y 点，初始值是 x0, y0 。然后循环迭代得到曲线上每一个新坐标点， xn, yn 并 计算出上一次迭代时点到此新点的距离， 然后再将新值赋值给 x, y。不我打算展示如何计算两点间的距离 ，我假定你已经有这样的函数了。将距离累加进 length.

最后再将算最后x2, y2 与 x, y 最后值的距离加到 length 上。然后返回 length 结果

确保你完全明白了，因为我打算在此处添加更多代码了。

我会在对曲线循环过程中追踪每个点之间的距离时非常有用。所以我们要把成功结果存进数组。这比返回整个长度有用，我们将直接返回这个数组
...
好的，有点儿小复杂， 让我们过一遍代码。我们用设 count 为 500 得到了长度数组，且获取了总长度。。

就像以前一样创建了一个 0.05 为步长 从 0 至 1 的循环。但并不像在画贝塞尔曲线时使用的 t， 我们用它寻找曲线长度的百分比。意思是当曲线长度为 500 像素且 i 为 0.5 时，我们寻找的的目标点长度即为 250 像素。

现在我们用循环遍历数组 通过 j 获取长度值直到它大于 targetLength 。我们将 j / count 得到这一部分的长度。我们 t 传入 quadBezierPoint 函数得到下一个点并把它绘制出来。此时我们应该跳出内部的循环，直到完成这些再入下一步循环。
...
最后一个点丢了（因为循环并未超过它的长度），但我们仅需要再绘制另一个坐标在 x2, y2 的点即可。这回相当接近均分了。 count 值设的越高，均分的精度就会越高。如果 count 降为 100 ，我们可以看下它的效果：
...
其次，我们添加了太多的点用于获取任意精度的点。因为在内循环中仅使用了我们抓取存储了我们找到的最后一个点。找到预先定义的点太费性能了， 我们可以在这个点与前一个点之间插值。
...
举个例子，假设我们的目标长度是150，然后在 index 87 位置我们得到了长度是值是 160。 index 往回到 86 我们得到 140。现在我希望得到 86 与 87 之间的值。 比起用 87 / count ， 或 86 / count ，我们用插值 50% 谓子 86.5 / count。虽然还是不太完美，但你现在可以将 count 设低一点，但得到结果却依旧很好。
...
如果你想获取更多这类相关的技术信息和完整解释，可以参考以下网站
