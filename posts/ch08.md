...
此处， for 环境内 t 从 res 到 finalT 变化所以不会画出完整的曲线（除非 finalT 为 1）。然后我们给 finalT 加上 dt。 这会让 finalT 慢慢接近 1， 这会曲线越来越完整。当 finalT 超过 1 时, 我们将它设为负值，这会让整个过程反转直到 finalT 变为 0， 这是我们弹回出发位置的另一种方式。
