...
一开始我本章标题我打算使用“次摆线与摆线（旋轮线）”。我想它们两个虽然为同，但却有联系。随着深入了解，它们两个我有点儿凌乱了。事实上摆线是一类非常特别的次摆线。我就不想这个问题了。直接放维基百科上它们的定义：
...
t 是圆的旋转角度，它可以无限增长， a 是圆的半径， b 是圆心点至绘制点的距离 - 你应该被称为那个点的半径。让我们把它实现出来：
...
我们将 a 和 b 周设为 20， 这样就可以得到摆线（译者者：普通旋转线）了。我们循环将从 0 至 canvas 的宽度用于变量 t, 应用公式连接至计算出的结果点。
...
这就是全部关于次摆线的内容。试试给 a 和 b 设不同的值，或者你自己改一下其它值，我就不再再多讲了。因为下面还留有很旋轮曲线需要探讨。
...
毫无意外，我们得到了一个半的结点。为了完成这个曲线，我们不得不再绕一圈，需要将 t 的范围值变为 0 到  PI * 4。然后得到结果：
...
蚶线是 长短幅摆线的一种，生成它的两个圆半径相等。如果两个点在圆内，则它是特殊的蚶线被称为 Cardioid 心形曲线。
...
注意 r0 与 r1 比例是 6:1, 拥有6个点。比例的作用原理与之前的长短幅外摆线一样

下面是半径312 和 76 (译者注：这里 r0 为312，r1，d 都为 76)：
...
小时候如果你像我一样比较书呆子气，那么你可能也会像我这样拥有下面这些东西：
...
这是几个星期前为写这一章内容我新买的。它挺酷的，锡盒子包装内包含了一些绘制用的纸还有使用说明/灵感小册子。
...
如果你将小齿轮摆外面滚一轮，它就是长短幅外摆线。相当有趣，说实施，我不是更喜欢用代码实现它。下面是绘制过程中笔滑了，这让长短辐外摆线看起来有点儿糟。笔触也断断续续的。
